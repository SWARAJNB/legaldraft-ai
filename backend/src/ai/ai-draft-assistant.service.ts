...
/**
 * Dynamic question trees for each legal draft type.
 * Each question has: key, label, placeholder, and whether it's required.
 */
const QUESTION_TREES: Record<
  string,
...
@Injectable()
export class AiDraftAssistantService {
  constructor(
  ...
  /**
   * Get available draft types for guided drafting.
   */
  ...
  /**
   * Start or continue a guided draft session.
   */
  async processGuidedDraft(
    userId: string,
    tenantId: string,
    params: { session_id?: string; draft_type?: string; answer?: string },
  ): Promise<{
    session_id: string;
    status: 'in_progress' | 'complete';
    current_question?: { key: string; label: string; placeholder: string; required: boolean; step: number; total: number };
    draft?: string;
    progress: number;
  }> {
    ...
    if (params.session_id) {
      ...
      if (!QUESTION_TREES[draftType]) {
        throw new NotFoundException(
          `Draft type '${params.draft_type}' is not supported for guided drafting. Available: ${Object.keys(QUESTION_TREES).join(', ')}`,
        );
      }
      ...
      throw new NotFoundException('Provide either session_id or draft_type');
    }

    const questions = QUESTION_TREES[conversation.draftType];
    if (!questions) {
      throw new NotFoundException('Invalid draft type in session');
    }
    ...
    const progress = Math.round(
      (conversation.currentStep / questions.length) * 100,
    );
